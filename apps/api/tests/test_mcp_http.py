...
def test_sdk_client_over_real_http(tmp_path):
    ...
    try:
        ...
        async def verify():
            async with httpx.AsyncClient(headers={"Authorization": "Bearer test-http-token"}, trust_env=False) as client:
                async with streamable_http_client(base + "/mcp", http_client=client) as (read, write, _):
                    async with ClientSession(read, write) as session:
                        ...
                        assert {"sonic_system_status", "sonic_generate_midi", "sonic_analyze_audio", "sonic_build_pack", "sonic_draft_release", "sonic_plan_session"} <= names
